...
static __tbe_net_node *tbe_net_get_ref(__tbe_net *a_net, unsigned int a_int_id);
...
static int tbe_net_cmp(const void *a_ptr1, const void *a_ptr2)
{
  ...
  return ((pnode1->int_id == pnode2->int_id) ? TBE_OK : TBE_FAILURE);
}
...
static int tbe_net_trav_dump1(const void *a_node, void *a_stream)
{
  ...
  return tbe_rlist_dump(pnode->rlist, pnode->int_id, pstrm);
}
...
static int tbe_net_trav_dump2_l2(const void *a_node, void *a_dump)
{
  ...
  rs = tbe_net_get_rel((tbe_net) pdump->net, pdump->int_id, pnode->int_id);
  
  fprintf(pdump->stream, "%03u ", pdump->int_id);
...
static int tbe_net_trav_prop1(const void *a_node, void *a_prop1)
{
  ...
  if (pnode->int_id == pprop->rel.int_id1)
    return TBE_OK;
  if (pnode->int_id == pprop->rel.int_id2)
    return TBE_OK;
  ...
  if (!TBE_NET_SKIP(rs1, pprop->rel.rs)) {
    ...
    if (rs2 != rs3) {
      retval = tbe_rqueue_enq1(pprop->queue,
                               pnode->int_id,
                               pprop->rel.int_id2,
                               rs3);
...
static int tbe_net_add_rel_noprop(__tbe_net *a_net, tbe_rel a_rel)
{
  ...
  if (!(pnode = tbe_net_get_ref(pnet, a_rel.int_id1)))
    return TBE_INVALIDINPUT;

  /* check if the second interval exists */
  if (!tbe_net_get_ref(pnet, a_rel.int_id2))
    return TBE_INVALIDINPUT;

  return tbe_rlist_add(pnode->rlist, a_rel.int_id2, a_rel.rs);
}
...
int tbe_net_add_int(tbe_net a_net, unsigned int a_int_id)
{
  ...
  if (tbe_net_get_ref(pnet, a_int_id))
...
int tbe_net_add_rel(tbe_net a_net, tbe_rel a_rel)
{
  ...
  retval = (tbe_rqueue_enq1(prop.queue, a_rel.int_id1, a_rel.int_id2, rs2));
...
int tbe_net_add_ep(tbe_net a_net, unsigned int a_int_id, tbe_interval a_int)
{
  ...
  if (!(pnode = tbe_net_get_ref(pnet, a_int_id)))
    return TBE_INVALIDINPUT;
                             ...
                             unsigned int a_int_id2)
{
  ...
  if (!(pnode = tbe_net_get_ref(pnet, int_id1)))
    return TBE_REL_SET_NUL;

  /* then we check if the larger interval exists */
  if (!tbe_net_get_ref(pnet, int_id2))
    return TBE_REL_SET_NUL;

  /* now we look for the larger interval in the rel list of the first */
  if (tbe_rlist_get(pnode->rlist, int_id2, &rs) != TBE_OK)
