		     ...
		     int hd) 
{
  ...
  if (type == PT_EXPR && left && !i1.paren && !left->i1.paren) {
	...
	fprintf(stderr, "\tmismatching operators: '%s' and '%s'.\n",
...
RuleType ParseNode::IdentifySpecialHead(int *nheads)
{
  ...
  if (!left || left->type == PT_ATOM)
    tp = BASICRULE;
  else if (left->type == PT_ORDERED_DISJUNCTION) {
...
{
  char indent_string[BUFFER_LENGTH];
  char *p;
...
int ParseNode::ProcessWeight()
{
  ...
  if (lt->has_range) {
    ...
    fprintf(stderr, "\toffending literal: ");
    lt->Print(1);
    fprintf(stderr, "\n");
  }
...
long ParseNode::Eval()
{
  ...
  try {
    ...
    fprintf(stderr, "%s: ", error_file_and_line(lineno));
    r.Print();
...
void ParseNode::PrintExpression()
{
  ...
  case PT_CONSTANT:
    ...
    break;
  case PT_NUMBER:
    fprintf(stderr, "%ld", l1.val);
    break;
  case PT_EXPR:
    if (l1.val != FUN_ABS) {
      if (left)
	left->PrintExpression();
      fprintf(stderr, "%s", operators[l1.val]);
      if (right)
