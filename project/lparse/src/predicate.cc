...
char *negation_name(char *nm)
{
  ...
  int underscores = 0;
  long pos = 0;
  static char neg_name[BUFFER_LENGTH] = { 0 };
  ...
  if (*nm == '-') {
    ...
    sprintf(&neg_name[pos], "%s", nm+1);
    neg_name[len-1] = '\0';
  } else {
    for (int i = 0; i < underscores; i++) {
      pos += sprintf(&neg_name[pos], "_");
    }
    sprintf(&neg_name[pos], "-%s", nm);
    neg_name[len+1] = '\0';
  }
  return neg_name;
}
...
void Predicate::DefineComplement(long p)
{
  ...
  dependency_graph->AddEdge(comp, p, true);
...
long Predicate::decode_atom(char *st, Instance *i)
{
  long pr = -1;
  long pos = 0;
  int paren_depth = 0;
...
void Predicate::CheckImplicitDomains()
{
  ...
  while ((lt = implicit_domain_list->Iterate())) {
    if (!lt->DomainLiteral()) {
      ...
      pos += sprintf(buf, error_template1,
	...
	pos += sprintf(&buf[pos], "%s/%d", predicates[p]->Name(), 
		predicates[p]->Arity());
      }
      pos += sprintf(&buf[pos], error_template2, lt->GetPrintString());
      buf[pos] = '\0';
      error(USR_ERR, buf);
	...
	pos += sprintf(&buf[pos], "%s/%d", predicates[p]->Name(),
      ...
      pos += sprintf(&buf[pos], error_template2,
		     lt->GetPrintString());
      buf[pos] = '\0';
      warn(WARN_DOMAIN, lt->lineno, buf);
    }
  }
}
