...
unsigned long SymbolTable::FindIndex(char *key, long mask)
{
...
long SymbolTable::Insert(char *key, int arity)
{
  ...
  char *name = new char[length+3+arity];
  strcpy (name, key);
  name[length] = '\0';
  ...
  if (arity > 0) {
    sprintf(&name[length], "/%d", arity);
  }
  // try to find the correct index
  index = FindIndex(name, NORMAL_MASK);
...
long SymbolTable::Lookup(char *key, int arity)
{
  ...
  long index = 0;
  static char name[BUFFER_LENGTH] = { 0 };
  int pos = 0;
  
  if (arity < 0) {
    pos = sprintf(name, key);
    name[pos] = '\0';
  } else {
  ...
  if (pos > BUFFER_LENGTH) {
    fprintf(stderr, "Buffer owerflow\n");
    abort();
  }
...
long SymbolTable::LookIndex(char *key, int arity)
{
  debug(DBG_SYMBOL, 2, "Looking for symbol (%s)", key);
  long index = 0;
  static char name[BUFFER_LENGTH] = { 0 };
...
{
  printf("This doesn't (at least currently) work\n");
}
