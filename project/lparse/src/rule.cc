...
void Rule::EmitSmodels2(int gpos, int gneg, int)
{
  Literal *lt = NULL;
...
void Rule::CalculateOptimize()
{
  ...
  if (sys_data.emit_text) {
    int first = 1;
    fprintf(sys_data.output_file, "%s %s ", (head->pred) ? "maximize"
...
char *Rule::GetInstanceString()
{
  ...
  if (label) {
    pos += sprintf(&buf[pos], "%s : ", label->GetInstanceString());
  }

  switch (type) {
  case BASICRULE:
    pos += sprintf(&buf[pos], "%s", head->GetInstanceString());
    break;
  case CHOICERULE:
    pos += sprintf(&buf[pos], "{ ");    
    for (i = 0; i < head_number; i++) {
  ...
  while (special_lits && (lt = special_lits->Iterate())) {
    ...
    pos += sprintf(&buf[pos], "%s", lt->GetInstanceString());
  }
  while ((lt = positive.Iterate())) {
    if (first) {
      first = 0;
...
char *Rule::GetPrintString()
{
  ...
  if (label) {
    pos += sprintf(&buf[pos], "%s : ", label->GetPrintString());
  }

  
  switch (type) {
  case BASICRULE:
    pos += sprintf(&buf[pos], "%s", head->GetPrintString());
    break;
  case CHOICERULE:
    pos += sprintf(&buf[pos], "{ ");    
    for (i = 0; i < head_number; i++) {
  ...
  while (special_lits && (lt = special_lits->Iterate())) {
    ...
    pos += sprintf(&buf[pos], "%s", lt->GetPrintString());
  }
  while ((lt = positive.Iterate())) {
    if (first) {
      first = 0;
  ...
  while ((fun = functions.Iterate())) {
    ...
    pos += sprintf(&buf[pos], "%s", fun->GetPrintString());
  }
  buf[pos++] = '.';
  buf[pos] = '\0';
...
void Rule::DefineImplicitDomain()
{
  ...
  if (type == BASICRULE) {
    head->AddVars(variables);
  } else {
    for (i = 0; i < head_number; i++) {
      head_array[i]->AddVars(variables);
    }
  }
  
  while ((lt = positive.Iterate())) {
    lt->AddVars(variables);
  }
  while ((lt = negative.Iterate())) {
    lt->AddVars(variables);
  }
  while ((fun = functions.Iterate())) {
    fun->AddVars(variables);
  }
