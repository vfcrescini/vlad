...
void Term::Print()
{
  ...
  case T_CONSTANT:
    if (IS_NUMBER(val)) {
      ...
      fprintf(stderr, "%s", constant_table->LookupByValue(val));
    }
    break;
  case T_VARIABLE:
    fprintf(stderr, "%s", variable_table->symbols[val]);
    break;
  case T_FUNCTION:
    ((Function*)this)->Print();
    break;
...
//        $1->PrintExpression();
//        fprintf(stderr, "' is not a constant valued expression.\n");
//      } 
//  	    if (!IS_NUMBER(lval2)) {
//                fprintf(stderr, "   expression '");
//                $3->PrintExpression();
...
Instance Function::Call()
{
  ...
  try {
    ...
    fprintf(stderr, "Runtime error at line %ld:", lineno );
    r.Print();
...
void Function::Print()
{
  ...
  for (i = 0; i < arity; i++) {
    ...
    case T_RANGE:
      r = (Range *) args[i];
      fprintf(stderr, "%ld..%ld", r->start->val, r->end->val);
      break;
    case T_ATOM:
...
void Range::Print()
{
  //  fprintf(stderr, "%ld .. %ld", start, end);
}
...
extern char *get_ground_instance(Instance *, long, int);
...
char *Function::GetPrintString()
{
  static char buf[BUFFER_LENGTH] = { 0 };
  int pos = 0, i = 0;
  pos += sprintf(buf, "%s(", name);
  ...
  for (i = 0; i < arity; i++) {
	...
	pos += sprintf(&buf[pos], "%ld", args[i]->val);
      }	else {
    ...
    case T_VARIABLE:
      pos += sprintf(&buf[pos], "%s",
		     variable_table->LookupByValue(args[i]->val));
      break;
    case T_FUNCTION:
      pos += sprintf(&buf[pos], "%s",
		     ((Function*)args[i])->GetPrintString());
      break;
    case T_ATOM:
      pos += sprintf(&buf[pos], "%s",
