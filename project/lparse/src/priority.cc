...
void PriorityData::GroundTester()
{
  ...
  if (sys_data.emit_text) {
    fprintf(sys_data.output_file, "%% Tester rules begin here\n");
  }
  ...
  if (sys_data.emit_text) {
    fprintf(sys_data.output_file, "%% Total number of "
	    "choices: %ld\n", num_choices);
  } else {
    fprintf(sys_data.output_file, "0\n");
  ...
  if (sys_data.emit_text) {
    fprintf(sys_data.output_file, "%% Interactive metapriorities "
  ...
  if (sys_data.emit_text) {
    fprintf(sys_data.output_file, "%% Rules corresponding to original choices\n");
  }
  long n = 1;
  char *st;
  ...
  while ((st = original_rules.Iterate())) {
    fprintf(sys_data.output_file, "%ld %ld %s\n",
	    n,
	    *choice_degrees[n],
	    st);
    n++;
  ...
  if (sys_data.use_interactive_priorities && !sys_data.emit_text) {
    ...
    while ((n = mp->atom.Iterate())) {
      f = mp->first.Iterate();
      s = mp->second.Iterate();
      fprintf(sys_data.output_file, "%s %ld %ld\n",
...
//        fprintf(sys_data.output_file, "%s: ", label_table->symbols[n]);
//        while ((ch = labels[n]->Iterate())) {
//  	fprintf(sys_data.output_file, "%ld ", ch);
//        }
