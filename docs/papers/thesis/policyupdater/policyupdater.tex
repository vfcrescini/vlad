...
\begin{document}
  ...
  query <gnd-auth-exp>;
      ...
      \subsection{Semantics}
        ...
        \subsubsection{Domain Description of Language ${\cal L^T}$}
        ...
        \begin{itemize}
          ...
          {\bf Entities}
            ...
            the set ${\cal E}$ that contains all the defined entities, as well
            as the subsets single subject ${\cal E}_{ss}$, single access right
            ${\cal E}_{as}$, single object ${\cal E}_{os}$, group subject
            ${\cal E}_{sg}$, group access right ${\cal E}_{ag}$ and group
            object ${\cal E}_{og}$.

          \item
          {\bf Intervals}

          The interval set ${\cal I}$ contains the set of all intervals.

          \item
          {\bf Atoms}

          Atoms of language ${\cal L^{T^*}}$ differs from atoms of language
          ${\cal L^T}$ in that the former also defines the state in which
          the atom holds.

          The set ${\cal A}^\sigma$ is the set of all atoms in state $\sigma$.
        ...
        \begin{itemize}
          ...
          \item
            ...
            described through the following problem. Given a set of unique
            variables $V$ = \{$v_0$, \ldots, $v_{|V|}$\}, find a set $T$ of
            $|V|$-tuples such that for each tuple \{$t_0$, \ldots, $t_{|V|}$\}
            in $T$, the type of each variable $v_i$ matches the type of the
            corresponding entity $t_i$, i.e. $Type$($v_i$) = $Type$($t_i$),
            where $0$ $\leq$ $i$ $\leq$ $|V|$ and the function
            $Type$($\upsilon$) returns the type (e.g. single subject, single
            access right, etc.) of the given variable or entity $\upsilon$.
            ...
            procedure, we must first define two functions: $Normalise$() and
            $Replace$().
            
            The $Normalise$($\epsilon$) function reorganises the interval
            expression input $\epsilon$ of the form $r_0$($i_0$, $j_0$),
            \ldots, $r_n$($i_n$, $j_n$), where $i$, $j$ $\in$ ${\cal I}$ and
            $r$ is one of {\em before}, {\em during}, etc. into a set of
            3-tuples \{\{$i_0$, $j_0$, $rs_0$\}, \ldots, \{$i_m$, $j_m$,
            $rs_m$\}\}, where each tuple is composed of a pair of intervals
            and a relation set. For example, given:
            ...
            \begin{vquote}
              $Normalise$($\epsilon$) =
                \{\{$i_0$, $i_1$, \{{\em before}, {\em starts}\}\},
                \{$i_1$, $i_2$, \{{\em before}\}\}\}.
            \end{vquote}
            
            The $Replace$($M$, $V$, $T$) function takes the following as input:
            a normalised set $M$ of interval relations with variables, a set of
            interval variables $V$ that occurs in $M$, and a tuple $T$ containing
            intervals that correspond to the variables in $V$. The function
            returns a set of interval relations similar to $M$, but with all
            variables in $V$ replaced with the corresponding intervals in $T$.
            For example:
            ...
            Using these two functions, we can now illustrate how to generate
            interval tuples that satisfy given temporal constraints. Given an
            interval expression $\epsilon$ and a set of variables $V$ that occurs
            in $\epsilon$, a set of interval tuples $T$ is said to satisfy
            $\epsilon$ if and only if each $t$ $\in$ $T$ in the form
          ...
          \item
            ...
            \begin{itemize}
              ...
              \item
                ...
                \begin{vquote}
                  $\forall$($s$, $a$, $o$, $\sigma$, $i_0$, $i_1$),
                
                  $\hat{holds}$($s$, $a$, $o$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{holds}$($s$, $a$, $o$, $\sigma$, $i_0$)
                
                  $\lnot$ $\hat{holds}$($s$, $a$, $o$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{holds}$($s$, $a$, $o$, $\sigma$, $i_0$)
              ...
              \item
                ...
                \begin{vquote}
                  $\forall$($s_s$, $s_g$, $\sigma$, $i_0$, $i_1$),

                  $\hat{memb}$($s_s$, $s_g$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{memb}$($s_s$, $s_g$, $\sigma$, $i_0$)

                  $\lnot$ $\hat{memb}$($s_s$, $s_g$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{memb}$($s_s$, $s_g$, $\sigma$, $i_0$)
                ...
                \begin{vquote}
                  $\forall$($a_s$, $a_g$, $\sigma$, $i_0$, $i_1$),

                  $\hat{memb}$($a_s$, $a_g$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{memb}$($a_s$, $a_g$, $\sigma$, $i_0$)

                  $\lnot$ $\hat{memb}$($a_s$, $a_g$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{memb}$($a_s$, $a_g$, $\sigma$, $i_0$)
                ...
                \begin{vquote}
                  $\forall$($o_s$, $o_g$, $\sigma$, $i_0$, $i_1$),

                  $\hat{memb}$($o_s$, $o_g$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{memb}$($o_s$, $o_g$, $\sigma$, $i_0$)

                  $\lnot$ $\hat{memb}$($o_s$, $o_g$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{memb}$($o_s$, $o_g$, $\sigma$, $i_0$)
              ...
              \item
                ...
                \begin{vquote}
                  $\forall$($s_{g0}$, $s_{g1}$, $\sigma$, $i_0$, $i_1$),

                  $\hat{subst}$($s_{g0}$, $s_{g1}$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{subst}$($s_{g0}$, $s_{g1}$, $\sigma$, $i_0$)

                  $\lnot$ $\hat{subst}$($s_{g0}$, $s_{g1}$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{subst}$($s_{g0}$, $s_{g1}$, $\sigma$, $i_0$)
                ...
                \begin{vquote}
                  $\forall$($a_{g0}$, $a_{g1}$, $\sigma$, $i_0$, $i_1$),

                  $\hat{subst}$($a_{g0}$, $a_{g1}$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{subst}$($a_{g0}$, $a_{g1}$, $\sigma$, $i_0$)

                  $\lnot$ $\hat{subst}$($a_{g0}$, $a_{g1}$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{subst}$($a_{g0}$, $a_{g1}$, $\sigma$, $i_0$)
                ...
                \begin{vquote}
                  $\forall$($o_{g0}$, $o_{g1}$, $\sigma$, $i_0$, $i_1$),

                  $\hat{subst}$($o_{g0}$, $o_{g1}$, $\sigma$, $i_1$) $\leftarrow$
                  $\hat{subst}$($o_{g0}$, $o_{g1}$, $\sigma$, $i_0$)

                  $\lnot$ $\hat{subst}$($o_{g0}$, $o_{g1}$, $\sigma$, $i_1$) $\leftarrow$
                  $\lnot$ $\hat{subst}$($o_{g0}$, $o_{g1}$, $\sigma$, $i_0$)
