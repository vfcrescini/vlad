...
\begin{document}
  ...
  update \(u\)()
    ...
    if \(\rho\sb{10\sb{0}}\), \(\ldots\), \(\rho\sb{10\sb{n\sb{10}}}\), \(!\)\(\rho\sb{11\sb{0}}\), \(\ldots\), \(!\)\(\rho\sb{11\sb{n\sb{11}}}\);
      ...
      \begin{vdefinition}
        ...
        $m$). An {\bf consistent instance} of interval network $N$ is an
      ...
      condition 2 in Definition \ref{def-cons1} above in two ways. Firstly, an
      inconsistent domain description can be caused by two or more conflicting
      interval constraint statements. This conflict occurs when the consequence
      of an interval constraint is implictly or explicitly excluded by another
      interval constraint. For example, the following two interval constraint
      statements are in conflict with each other because the second defines a
      relation that has been explicitly excluded by the first.
  ...
  relation equals(i0, i1);
      ...
      not. To illustrate this type of inconsistency, we use Allen's \cite{AL}
      example, shown in Figure \ref{fig-net-ex-07}.
      ...
      The network shown above is inconsistent, as no consistent instance of
      this network exists. 
