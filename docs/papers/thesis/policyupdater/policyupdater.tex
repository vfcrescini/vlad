...
\begin{document}
  \onehalfspacing
  \title{
    Implementation of a Logic-Based Access Control System with
    Temporal Constraints
  }
  \author{Vino Fernando Crescini}
  \date{April 2006}
  ...
  \maketitle
  \tableofcontents
  \listof{valgorithmfloat}{List of Algorithms}
  \listoffigures
  \listoftables
  \newpage
  ...
  \chapter{PolicyUpdater System}
    ...
    \section{System Structure}
      ...
      \subsection{Data Structures}
        ...
        type or another list type. This list structure implements, among
        others, the following operations:

        \begin{itemize}
          \item
            $LIST$.$len$() returns the number of elements in $LIST$.

          \item
            $LIST$.$is_in$($item$) returns true if $item$ is in the list and
            false otherwise.

          \item
            $LIST$.$index\_of$($item$) returns the index of the item $item$
            in $LIST$.
            ...
            \begin{table}[tbhp]
              \begin{center}
                \begin{tabular}[t]{|l|l|l|}
                  ...
                  \hline
                  $pcnd$         & expression type & positive premise \\
                  \hline
                  $ncnd$         & expression type & negative premise \\
                  \hline
                \end{tabular}
              \end{center}
    ...
    with absence
      ...
      \subsection{Translation to Normal Logic Program}
        ...
        \subsubsection{Generating the Normal Logic Program from the Policy Base}
          ...
          $Ts$, Initial State Facts Table $Ti$, Constraint Rules Table $Tc$,
          Policy Update Definition Table $Tu$, and Policy Update Sequence Table
          $Tq$:

          \begin{valgorithm}[tbhp]{GenNLP()}{algo-polup-genlp}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{TransInitStateRules()}{algo-polup-tisr}
FUNCTION \(TransInitStateRules\)(\(Ti\))
  FOR \(i\) = \(0\) TO \(|Ti|\) DO
    \(RuleBegin\)()
    \(RuleHead\)(\(Encode\)(\(Ti\)[\(i\)].\(atm\), \(0\), \(Ti\)[\(i\)].\(tr\)))
    \(RuleBody\)(\(true\), \(true\))
    \(RuleEnd\)()
  ENDDO
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{TransConstRules()}{algo-polup-tconr}
FUNCTION \(TransConstRules\)(\(Tc\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|Tc|\) DO
      \(RuleBegin\)()
      FOR \(k\) = \(0\) TO \(|\)\(Tc\)[\(j\)].\(exp\)\(|\) DO
        \(a\) = \(Encode\)(\(Tc\)[\(j\)].\(exp\)[\(k\)].\(atm\), \(i\), \(Tc\)[\(j\)].\(exp\)[\(k\)].\(tr\))
        \(RuleHead\)(\(a\))
      ENDDO
      FOR \(k\) = \(0\) TO \(|\)\(Tc\)[\(j\)].\(pcnd\)\(|\) DO
        \(a\) = \(Encode\)(\(Tc\)[\(j\)].\(pcnd\)[\(k\)].\(atm\), \(i\), \(Tc\)[\(j\)].\(pcnd\)[\(k\)].\(tr\))
        \(RuleBody\)(\(a\), \(true\))
      ENDDO
      FOR \(k\) = \(0\) TO \(|\)\(Tc\)[\(j\)].\(ncnd\)\(|\) DO
        \(a\) = \(Encode\)(\(Tc\)[\(j\)].\(ncnd\)[\(k\)].\(atm\), \(i\), \(Tc\)[\(j\)].\(ncnd\)[\(k\)].\(tr\))
        \(RuleBody\)(\(a\), \(false\))
      ENDDO
      \(RuleEnd\)()
    ENDDO
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{TransUpdateRules()}{algo-polup-tupdr}
FUNCTION \(TransUpdateRules\)(\(Tu\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|Tu|\) DO
      IF \(Tq\)[\(i\)].\(name\) == \(Tu\)[\(j\)].\(name\) THEN
        \(e\) = \(GndUpdate\)(\(Tu\)[\(j\)], \(Tq\)[\(i\)].\(ilist\))
        \(RuleBegin\)()
        FOR \(k\) = \(0\) TO \(|\)\(e\).\(pst\)\(|\) DO
          \(a\) = \(Encode\)(\(e\).\(pst\)[\(k\)].\(atm\), \(i\) + \(1\), \(e\).\(pst\)[\(k\)].\(tr\))
          \(RuleHead\)(\(a\))
        ENDDO
        FOR \(k\) = \(0\) TO \(|\)(\(e\).\(pre\)\(|\) DO
          \(a\) = \(Encode\)(\(e\).\(pre\)[\(k\)].\(atm\), \(i\), \(e\).\(pre\)[\(k\)].\(tr\))
          \(RuleBody\)(\(a\), \(true\))
        ENDDO
        \(RuleEnd\)()
      ENDIF
...
ENDFUNCTION
          ...
          The function $GndUpdate$($U$, $IL$) used in Algorithm
          ...
          \begin{valgorithm}[tbhp]{GenInherRules()}{algo-polup-ginhr}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenSubSubstInherRules()}{algo-polup-gssir}
FUNCTION \(GenSubSubstInherRules\)(\(Ts\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|\)\(Ts\).\(sg\)\(|\) DO
      FOR \(k\) = \(0\) TO \(|\)\(Ts\).\(sg\)\(|\) DO
        IF \(Ts\).\(sg\)[\(j\)] != \(Ts\).\(sg\)[\(k\)] THEN
          FOR \(l\) = \(0\) TO \(|\)\(Ts\).\(a\)\(|\) DO
            FOR \(m\) = \(0\) TO \(|\)\(Ts\).\(o\)\(|\) DO
              \(RuleBegin\)()
              \(h\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(ag\)[\(l\)], \(Ts\).\(og\)[\(m\)]\}
              \(RuleHead\)(\(Encode\)(\(h\_atm\), \(i\), \(true\)))
              \(h\_atm\) = \{\(Ts\).\(sg\)[\(k\)], \(Ts\).\(ag\)[\(l\)], \(Ts\).\(og\)[\(m\)]\}
              \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\), \(true\)), \(true\))
              \(s\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(sg\)[\(k\)]\}
              \(RuleBody\)(\(Encode\)(\(s\_atm\), \(i\), \(true\)), \(true\))
              \(h\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(ag\)[\(l\)], \(Ts\).\(og\)[\(m\)]\}
              \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\), \(false\)), \(false\))
              \(RuleEnd\)()
              \(RuleBegin\)()
              \(h\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(ag\)[\(l\)], \(Ts\).\(og\)[\(m\)]\}
              \(RuleHead\)(\(Encode\)(\(h\_atm\), \(i\), \(false\)))
              \(h\_atm\) = \{\(Ts\).\(sg\)[\(k\)], \(Ts\).\(ag\)[\(l\)], \(Ts\).\(og\)[\(m\)]\}
              \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\), \(false\)), \(true\))
              \(s\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(sg\)[\(k\)]\}
              \(RuleBody\)(\(Encode\)(\(s\_atm\), \(i\), \(true\)), \(true\))
              \(RuleEnd\)()
            ENDDO
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenTransRules()}{algo-polup-gtrar}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenSubTransRules()}{algo-polup-gstrr}
FUNCTION \(GenSubTransRules\)(\(Ts\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|\)\(Ts\).\(sg\)\(|\) DO
      FOR \(k\) = \(0\) TO \(|\)\(Ts\).\(sg\)\(|\) DO
        FOR \(l\) = \(0\) TO \(|\)\(Ts\).\(sg\)\(|\) DO
          IF \(j\) != \(k\) AND \(j\) != \(l\) AND \(k\) != \(l\) THEN
            RuleBegin()
            \(s\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(sg\)[\(l\)]\}
            \(RuleHead\)(\(Encode\)(\(s\_atm\), \(i\), \(true\)))
            \(s\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(sg\)[\(k\)]\}
            \(RuleBody\)(\(Encode\)(\(s\_atm\), \(i\), \(true\)), \(true\))
            \(s\_atm\) = \{\(Ts\).\(sg\)[\(k\)], \(Ts\).\(sg\)[\(l\)]\}
            \(RuleBody\)(\(Encode\)(\(s\_atm\), \(i\), \(true\)), \(true\))
            \(RuleEnd\)()
          ENDIF
        ENDDO
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenInertRules()}{algo-polup-giner}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenHldsInertRules()}{algo-polup-ghinr}
FUNCTION \(GenHldsInertRules\)(\(Ts\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|\)\(Ts\).\(s\)\(|\) DO
      FOR \(k\) = \(0\) TO \(|\)\(Ts\).\(a\)\(|\) DO
        FOR \(l\) = \(0\) TO \(|\)\(Ts\).\(o\)\(|\) DO
          \(h\_atm\) = \{\(Ts\).\(s\)[\(j\)], \(Ts\).\(a\)[\(k\)], \(Ts\).\(o\)[\(l\)]\}
          \(RuleBegin\)()
          \(RuleHead\)(\(Encode\)(\(h\_atm\), \(i\) \(+\) \(1\), \(true\)))
          \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\), \(true\)), \(true\))
          \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\) \(+\) \(1\), \(false\)), \(false\))
          \(RuleEnd\)()
          \(RuleBegin\)()
          \(RuleHead\)(\(Encode\)(\(h\_atm\), \(i\) \(+\) \(1\), \(false\)))
          \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\), \(false\)), \(true\))
          \(RuleBody\)(\(Encode\)(\(h\_atm\), \(i\) \(+\) \(1\), \(true\)), \(false\))
          \(RuleEnd\)()
        ENDDO
      ENDDO
    ENDDO
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenRefleRules()}{algo-polup-grefr}
FUNCTION \(GenRefleRules\)(\(Ts\), \(Tq\))
  \(GenSubRefleRules\)(\(Ts\), \(Tq\))
  \(GenAccRefleRules\)(\(Ts\), \(Tq\))
  \(GenObjRefleRules\)(\(Ts\), \(Tq\))
ENDFUNCTION
          \end{valgorithm}

          \begin{valgorithm}[tbhp]{GenSubRefleRules()}{algo-polup-gsrer}
FUNCTION \(GenSubRefleRules\)(\(Ts\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|\)\(Ts\).\(sg\)\(|\) DO
      \(s\_atm\) = \{\(Ts\).\(sg\)[\(j\)], \(Ts\).\(sg\)[\(j\)]\}
      \(RuleBegin\)()
      \(RuleHead\)(\(Encode\)(\(s\_atm\), \(i\), \(true\)))
      \(RuleBody\)(\(true\), \(true\))
      \(RuleEnd\)()
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenConsiRules()}{algo-polup-gconr}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{GenHldsConsiRules()}{algo-polup-ghcor}
...
ENDFUNCTION
      ...
      \subsection{Query Evaluation}
        ...
        \footnotetext{
          Smodels Package from {\tt \scriptsize http://www.tcs.hut.fi/Software/smodels}
        }
        ...
        a state $S$, a query expression $Qe$ can be evaluated against a list of
        stable models $SM$, where each element in $SM$ is a list of facts.

        \begin{valgorithm}[tbhp]{EvaluateExp()}{algo-polup-evale}
...
ENDFUNCTION
        ...
        \ref{algo-polup-evalf} evaluates a single fact $Qf$ in state $S$,
        against a list of stable models $SM$.

        \begin{valgorithm}[tbhp]{EvaluateFact()}{algo-polup-evalf}
FUNCTION \(EvaluateFact\)(\(Qf\), \(SM\), \(S\))
  IF \(IsFactIn\)(\(Encode\)(\(Qf\).\(atm\), \(S\), \(Qf\).\(tr\)), \(SM\)) THEN
    RETURN \(true\)
  ELSE
    IF \(IsFactIn\)(\(Encode\)(\(Qf\).\(atm\), \(S\), NOT \(Qf\).\(tr\)), \(SM\)) THEN
      RETURN \(false\)
    ELSE
      RETURN \(unknown\)
    ENDIF
...
ENDFUNCTION
        ...
        \begin{valgorithm}[tbhp]{IsFactIn()}{algo-polup-isinf}
FUNCTION \(IsFactIn\)(\(Fi\), \(SM\))
  FOR \(i\) = \(0\) TO \(|SM|\) DO
    IF NOT \(SM\)[\(i\)].\(is\_in\)(\(Fi\)) THEN
      RETURN \(false\)
    ENDIF
  ENDDO
  RETURN \(true\)
    ...
    \section{Allen's Temporal Interval Algebra}
      ...
      \subsection{Inferring New Relations}
        ...
        \subsubsection{Propagation Algorithm}
          ...
          \begin{valgorithm}[tbhp]{Trans$_2$()}{algo-tempo-trns2}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{NET.Add()}{algo-tempo-ntadd}
...
ENDFUNCTION
    ...
    \section{Extensions to Allen's Interval Algebra}
      ...
      \subsection{Defining Intervals in Terms of Time Points}
        ...
        \begin{valgorithm}[tbhp]{Compute()}{algo-tempo-compu}
...
ENDFUNCTION
        ...
        \begin{valgorithm}[tbhp]{Net.Bind()}{algo-tempo-ntbin}
    ...
    \section{Temporal Reasoner}
      ...
      \subsection{Network Structure}
        ...
        \subsubsection{Binary Representation of Relation Sets}
          ...
          \footnotetext{A similar method is used in \cite{VAN, VIL}.}
        ...
        \subsubsection{Network Data Structure}
          ...
          Algorithm \ref{algo-impln-norml} before adding a relation to the
          network:

          \begin{valgorithm}[tbhp]{Normalise()}{algo-impln-norml}
...
ENDFUNCTION
    ...
    \section{Authorisation Engine}
      ...
      \subsection{Translation to Normal Logic Program}
        \label{subs-impln-trans}

        The method used for translating a language ${\cal L^T}$ domain
        into a normal logic program is similar to the language ${\cal L}$
        domain translation discussed in Section \ref{subs-polup-norlp}.
        Obviously, translation of a language ${\cal L^T}$ domain requires
        extra mechanisms to handle temporal intervals. In this section, focus
        shall be given to these extensions in a more implementation oriented
        approach.

        \subsubsection{Encode() Function Revisited}
          ...
          \begin{valgorithm}[tbhp]{Encode()}{algo-impln-encod}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{Offset\_Holds()}{algo-impln-offho}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{Offset\_Memb()}{algo-impln-offme}
...
ENDFUNCTION
          ...
          \begin{valgorithm}[tbhp]{Offset\_Subst()}{algo-impln-offsu}
...
ENDFUNCTION
        ...
        \subsubsection{Extending GenNLP()}

          The $GenNLP$() function in Algoritm \ref{algo-polup-genlp} and the
          rest of the functions in Section \ref{subs-polup-norlp} require only
          little modifications to generate a normal logic program from a
          language ${\cal L^T}$ domain. In fact, the only major difference is
          that the new $GenNLP$() function must support the language
          ${\cal L^T}$ atoms that includes an additional temporal interval
          parameter. This means that most of the rule-generationg functions
          are extended simply by the addition of an extra interval loop, which
          passes intervals to the new $Encode$() function discussed in the
          previous section.

          However, as Section \ref{subs-tempo-seman} have shown,
          translating a language ${\cal L^T}$ domain to a normal logic program
          requires the generation of temporal relation rules. The semantics of
          the language tells us that the purpose of these rules is to assert
          that an atom that holds in a particular time interval $\iota$ must
          also hold in every other interval $\iota'$ that is "within" $\iota$,
          that is, $\iota'$ is {\em equal}, is {\em during}, {\em starts} or
          {\em finishes} $\iota$.

          Algorithm \ref{algo-impln-gente} shows the $GenTempoRules$() function
          which will be called by our extended $GenNLP$() function. $GenNLP$()
          takes three parameters as input: $NET$, the interval relation network
          structure; $Ts$, the symbol table; and $Tq$, the policy update
          sequence table. These parameters are passed to the three functions
          that generate the three types of temporal rules which correspond to
          ...
          \begin{valgorithm}[tbhp]{GenTempoRules()}{algo-impln-gente}
FUNCTION \(GenTempoRules\)(\(NET\), \(Ts\), \(Tq\))
  \(GenHldsTempoRules\)(\(NET\), \(Ts\), \(Tq\))
  \(GenMembTempoRules\)(\(NET\), \(Ts\), \(Tq\))
  \(GenSubsTempoRules\)(\(NET\), \(Ts\), \(Tq\))
ENDFUNCTION
          \end{valgorithm}

          \begin{valgorithm}[tbhp]{GenHldsTempoRules()}{algo-impln-ghdtr}
FUNCTION \(GenHldsTempoRules\)(\(NET\), \(Ts\), \(Tq\))
  FOR \(i\) = \(0\) TO \(|Tq|\) DO
    FOR \(j\) = \(0\) TO \(|\)\(Ts\).\(s\)\(|\) DO
      FOR \(k\) = \(0\) TO \(|\)\(Ts\).\(a\)\(|\) DO
        FOR \(l\) = \(0\) TO \(|\)\(Ts\).\(o\)\(|\) DO
          FOR \(m\) = \(0\) TO \(|\)\(Ts\).\(int\)\(|\) DO
            FOR \(n\) = \(0\) TO \(|\)\(Ts\).\(int\)\(|\) DO
              \(rs\) = \(NET\).\(get\)(\(m\), \(n\))
              IF (\(rs\) | \(101\)) == \(101\) THEN
                \(a\) = \{\(Ts\).\(s\)[\(j\)], \(Ts\).\(a\)[\(k\)], \(Ts\).\(o\)[\(l\)], \(Ts\).\(int\)[\(n\)]\}
                \(b\) = \{\(Ts\).\(s\)[\(j\)], \(Ts\).\(a\)[\(k\)], \(Ts\).\(o\)[\(l\)], \(Ts\).\(int\)[\(m\)]\}
                \(RuleBegin\)()
                \(RuleHead\)(\(Encode\)(\(a\), \(i\), \(true\)))
                \(RuleBody\)(\(Encode\)(\(b\), \(i\), \(true\)), \(true\))
                \(RuleEnd\)()
                \(RuleBegin\)()
                \(RuleHead\)(\(Encode\)(\(a\), \(i\), \(false\)))
                \(RuleBody\)(\(Encode\)(\(b\), \(i\), \(false\)), \(true\))
                \(RuleEnd\)()
              ENDIF
          ENDDO
        ENDDO
