    ...
    \section{Allen's Temporal Interval Algebra}
      ...
      \subsection{Inferring New Relations}
        ...
        \subsubsection{Propagation Algorithm}
          ...
          there is an arc that connects it to every other node. The algorithm is
          invoked whenever new information is to be added into the network. Whenever
          a new relation is added into the network, all consequences of this new
          relation are also added into the network. These consequences are computed
          through the transitive closure of the network. The following example
...
ENDFUNCTION
    ...
    \section{Extensions to Allen's Interval Algebra}
      ...
      \subsection{Time Points Revisited}
        ...
        arbitrarilty. Idealy, for a specific application, one would choose a
        granularity that is at least as small as the smallest interval in that
        application. In practice, however, one can choose a granularity that would
        encompass the largest interval that one needs to treat as an event.
        ...
        event that occurres at a specific time point. A granularity of 1 second,
        ...
        point in time. We can therefore argue that given a specific time granularity,
        some events can be treated as instantaneously occuring at a specific time
        point.
