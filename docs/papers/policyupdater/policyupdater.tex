...
query holds(alice, read, file);
    ...
    \subsection{Semantics}
      ...
      \subsubsection{Language ${\cal L}^{*}$}
        ...
        set of facts and constraints are generated. To precisely define the
        underlying semantics of domain description ${\cal D}_{\cal L}$ in
        language ${\cal L}$, we introduce language ${\cal L}^{*}$, which is an
        extended logic program representation of langauge ${\cal L}$, with
      ...
      \subsubsection{Translating Language ${\cal L}$ to Language ${\cal L^{*}}$}
        ...
        \paragraph{}
          ...
          Note that language ${\cal L}^{*}$ contains only one special state
          constant $S_{0}$ to represent the initial state of a given domain
          description. All other states are represented as a resulting state
          obtained by applying the $Result$ function.
  ...
  initially \(f\sb{0}\) && ... && \(f\sb{n}\)
          ...
          \begin{quote}
            ...
            where $\hat{f_{i}}$ $=$ $TransFact$($f_{i}$, $S_{0}$), $0$ $\leq$
            $i$ $\leq$ $n$
          \end{quote}
