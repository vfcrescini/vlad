...
query holds(alice, read, file);
  ...
  \section{Implementation}
    ...
    \subsection{Data Structures}
      ...
      be stored in a buffer before the translation process is started.
      ...
      \subsubsection{Symbol Table}
        ...
        \begin{itemize}
          \item
            Single Subject Entity List ($\cal E_{SS}$)
          \item
            Group Subject Entity List ($\cal E_{SG}$)
          \item
            Single Access Right Entity List ($\cal E_{AS}$)
          \item
            Group Access Right Entity List ($\cal E_{AG}$)
          \item
            Single Object Entity List ($\cal E_{OS}$)
          \item
            Group Object Entity List ($\cal E_{OG}$)
        \end{itemize}
 
        In addition to the above lists, the symbol table also defines 3
        conceptual pseudo-lists that are concatenations of the above lists:
 
        \begin{itemize}
          \item
            Subject Entity List ($\cal E_{S}$ = $\cal E_{SS} + \cal E_{SG}$)
          \item
            Access Right Entity List ($\cal E_{A}$ = $\cal E_{AS} + \cal E_{AG}$)
          \item
            Object Entity List ($\cal E_{O}$ = $\cal E_{OS} + \cal E_{OG}$)
        \end{itemize}
 
        Each entity identifier in the above lists are sorted accoring to their
        type, and ordered accoring to the order in which they are declared in the
        program. Each list is indexed by consecutive positive integers starting
        from zero. Pseudo-lists are ordered in the same manner as the other
        lists, with the single list elements starting at index 0, and the group
        list elements offset by the length of the single list.
      ...
      \subsubsection{Atoms, Facts and Expressions}
        ...
        \paragraph{Atoms.}
          ...
          \subparagraph{Holds}
            \begin{itemize}
              \item
                Subject Entity Identifier String
              \item
                Access Entity Right Identifier String
              \item
                Object Entity Identifier String
            \end{itemize}
 
          \subparagraph{Member}
            \begin{itemize}
              \item
                Single Entity Identifier String
              \item
                Group Entity Identifier String
            \end{itemize}
  
          \subparagraph{Subset}
            \begin{itemize}
              \item
                Subgroup Entity Identifier String
              \item
                Supergroup Entity Identifier String
            \end{itemize} 

        \paragraph{Facts.}
          A fact is a three-element structure composed of a polymorphic type
          which can be any of the three atom structures above; a type
          indicator to specify whether the fact is holds, member or subset;
          and a truth flag, to indicate whether the atom is true or false.
 
          \begin{itemize}
            \item
              polymorphic atom structure
            \item
              atom type indicator (holds$|$member$|$subset)
            \item
              truth flag (true$|$false)
          \end{itemize}
 
        \paragraph{Expressions.}
 
          Since an expression is simply a conjunction of facts, they are
          represented as an unordered list of fact structures.

      \subsubsection{Policy Base Tables}

        When a language ${\cal L}$ program is parsed, each of the facts, rules
        and policy updates must be stored into the policy base tables before
        ...
        \paragraph{Initial State Facts Table.}
          ...
          statements are added into the initial state expression.
        ...
        \paragraph{Constraint Table.}
          ...
          \begin{itemize}
            \item
              Expression representing the consequent
            \item
              Expression representing the positive premise
            \item
              Expression representing the negative premise
          \end{itemize}
 
        \paragraph{Policy Update Table.}
          Another list of structures is the policy update table. Each element
          structure of this table is composed of the following 4 sub elements:
 
          \begin{itemize}
            \item
              Policy update identifier
            \item
              Ordered string list of variable identifier placeholders
            \item
              Expression representing the precondition
            \item
        ...
        \paragraph{Policy Update Sequence Table.}
          ...
          \begin{itemize}
            \item
              Polcy update identifier
            \item
              Ordered string list of entity identifiers to replace variables
          \end{itemize}
