    ...
    with absence
      ...
      \subsubsection{Translation to Normal Logic Program.}
        ...
        \paragraph{Generating the Normal Logic Program from the Policy Base.}
          ...
          then be generated for evaluation. The following algorithms generate
...
FUNCTION TransUpdateRules(Tu, Tq)
  FOR i = 0 TO Len(Tq) DO
    FOR j = 0 TO Len(Tu) DO
      GroundUpdate(Tu[j], Tq)
      RuleBegin()
      FOR k = 0 TO Len(Tu[j].post) DO
        a = Encode(Tu[j].post[k].atm,
                   i + 1,
                   Tu[j].post[k].tr)
        RuleHead(a, T)
      ENDDO
      FOR k = 0 TO Len(Tu[j].pre) DO
        a = Encode(Tu[j].pre[k].atm,
                   i,
                   Tu[j].pre[k].tr)
        RuleBody(a, T)
      ENDDO
...
ENDFUNCTION
          ...
          The function $GroundUpdate()$ above simply replaces all occurences
          of variables in all the facts in the given policy update statement
          with the corresponding entities in the given policy update sequence
          table.
...
ENDFUNCTION
          ...
          The function $GenSubSubstInherRules()$ above generates the
          subject subset inheritance rules. Similar techniques are used for
          the generation of other subset and membership inheritance rules and
          are therefore omitted.

          The function below generates all the transitivity rules. As similar
          techniques are used to generate the subject, access right and and
          object transitivity rules, only the function that generates the
          subject transitivity rules is shown.
...
ENDFUNCTION
          ...
          Inertial rules are generated by the function below. As the algorithm
          to generate holds, membership and subset inertial rules are similar
          to each other, only the function that generates holds inertial rules
          is shown.
    ...
    FOR j = 0 TO Len(Ts.s) DO
      FOR k = 0 TO Len(Ts.a) DO
        FOR l = 0 TO Len(Ts.o) DO
          ...
          RuleBegin()
          a = Encode(ahlds, i + 1, T)
          RuleHead(a, T)
          a = Encode(ahlds, i, T)
          RuleBody(a, T)
          a = Encode(ahlds, i + 1, F)
          RuleBody(a, F)
          RuleEnd()
...
ENDFUNCTION
          ...
          rules. As with the other rule types, only the function that generates
          the subject identity rules is shown.
...
ENDFUNCTION
          ...
          \begin{vverbatim}
FUNCTION GenSubIdentRules(Ts, Tq)
  FOR i = 0 TO Len(Tq) DO
    FOR j = 0 TO Len(Ts.sg) DO
      asubst.grp1 = Ts.sg[j]
      asubst.grp2 = Ts.sg[j]
...
ENDFUNCTION
          ...
          The last two functions below shows the algorithm to generate the
          consistency rules.
...
ENDFUNCTION
    ...
    \subsection{Implementation Results}
      ...
      In this sub-section, we investigate the effects of domain size over
      ...
      The language L code listing in Example \ref{ex-1} is used in the first
      test case. In the second test case, the same code is used with an
      additional 20 new singular entities and 20 new group entities. Test
      cases 3 and 4 are similar to test case 1, except 100 new
      singular and group entities were added, respectively. Test cases 5 and 6
      are similar to test case 2, except 100 new initial state facts and
      constraint rules were added, respectively. In test case 7, 100 new policy
      update definitions were added, and in test case 8, these policy update
      defintions were applied. Test case 9 is similar to test case 2, but this
      one tries to evaluate 100 additional query facts. Finally, test case 11
      is a combination of test cases 6 and 8.
      ...
      L statements to a normal logic program and to generate the answer sets.
      $T_{Q}$ is the total time (in seconds) used by the system to evaluate
      all the queries. To increase result accuracy, each test was conducted 10
      times. The figures below show the average computation times.
      ...
      computation times were low because only one policy update is actually
      applied.
      ...
      are translated directly into normal logic program rules. Test case 11,
      on the other hand, shows a significant increase in both computation and
