...
\begin{document}
  ...
  \begin{abstract}
    The {\em PolicyUpdater}\footnotemark system is an access control system
    that provides policy evaluations as well as dynamic policy updates. These
    are achieved by the use of a logic-based language ${\cal L}$ to represent
    the underlying access control policies, constraints and update
    propositions. The system performs authorisation query evaluations and
    conditional policy updates by first translating the language $\cal{L}$ to a
    normal logic program in a form suitable for evaluation using the
    {\em Stable Model} semantics.
  \end{abstract}

  \footnotetext{PolicyUpdater home page at \\
                \tt http://www.cit.uws.edu.au/\~{}jcrescin/PolicyUpdater}
  ...
  \section{Introduction}
    ...
    perform a sequence of policy updates, and a means to evaluate queries
    against a policy or updated policies.
    ...
    updates. Appendix \ref{app-store} shows detailed specifications of the
    data structures used by the system. The case study presented in Appendix
    \ref{app-case} shows a typical application of the PolicyUpdater system:
    an access control system for web servers.
...
query holds(alice, read, file);
  ...
  always holds(SSUB, write, file)
    ...
    with absence
      ...
      \subsubsection{Translation to Normal Logic Program}
        ...
        the {\em Stable Model} semantics. However, the implementation of such
  ...
  \section{Conclusion}
    ...
    Because language ${\cal L}$ is a first-order logic language, the underlying
    semantics allow the language to be translated into an extended logic
    program, and then into a normal logic program through the application of
    the rules discussed in this paper. Through these translation rules we have
    also shown that dynamic and conditional policy updates can be achieved by
    the application of translated constraint rules.
    ...
    extension to the system and language to include a more flexible conflict
    resolution strategy to replace the system's current
    {\em negation-has-higher-precedence} resolution rule.
  ...
  \begin{thebibliography}{5}
    ...
    \bibitem{BE2}
      ...
      in {\em Proceedings of IJCAI Workshop for Nonmonotonic Reasoning,
      Action and Change},
      pp. 37-44, 2003
    ...
    \bibitem{LOB}
      ...
      in {\em Proceedings of AAAI 16th National Conferfence on Artificial
