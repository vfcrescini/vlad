...
\begin{document}
  ...
  \section{Web Server Access Control}
    ...
    request for a file; (2) the access control modules check if a policy is
    associated with the resource, then evaluates the policy to decide whether
    to allow or deny the request; (3) if the request is allowed, it is then
    passed on to other request handlers for processing; and (4) eventually, the
    transaction is completed by sending the requested file back to the client.
