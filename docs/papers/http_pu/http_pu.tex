...
\begin{document}
  ...
  \section{Web Server Access Control}
    ...
    The diagram in figure [something] illustrates how access control is
    enforced in a simplified HTTP request. (1) client makes a {\em GET}
    request for file $f$; (2) the access control modules check if a policy is
    associated with the resource; (3) the modules evaluates the policy and
    decides whether to allow or deny the request; (4) if the request is
    allowed, it is then passed on to other request handlers for processing;
    and (5) eventually, the transaction is completed by sending the requested
    file $f$ back to the client.
