  ...
  switch(a_type) {
    ...
    case VLAD_ATOM_SUBSET :
      ...
      return
        ...
        (VLAD_LEN_AG * VLAD_LEN_AG) +
        (VLAD_LEN_OG * VLAD_LEN_OG);
                                        ...
                                        bool a_truth)
{
  ...
  return
    compute_fact((a_sub * (VLAD_LEN_AS + VLAD_LEN_AG) * (VLAD_LEN_OS + VLAD_LEN_OG)) +
                 (a_acc * (VLAD_LEN_OS + VLAD_LEN_OG)) +
                 (a_obj),
                 a_state,
                 a_truth);
}
                              ...
                              char **a_obj)
{
  ...
  rem = a_id % ((VLAD_LEN_AS + VLAD_LEN_AG) * (VLAD_LEN_OS + VLAD_LEN_OG));
  index[0] = a_id / ((VLAD_LEN_AS + VLAD_LEN_AG) * (VLAD_LEN_OS + VLAD_LEN_OG));
...
int vlad_mapper::decode_memb(unsigned int a_id, char **a_elt, char **a_grp)
{
 int retval;
...
int vlad_mapper::decode_subst(unsigned int a_id, char **a_grp1, char **a_grp2)
{
  int retval;
